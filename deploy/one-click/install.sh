...
if [[ -n "${CUBE_SANDBOX_NETWORK_CIDR}" ]]; then
  check_cidr_preflight "${CUBE_SANDBOX_NETWORK_CIDR}" "${cidr_skip_conflict}" "CUBE_SANDBOX_NETWORK_CIDR" 29
  export CUBE_SANDBOX_NETWORK_CIDR
else
  check_cidr_preflight "192.168.0.0/18" "${cidr_skip_conflict}" "default CubeSandbox network CIDR" 29
fi
if [[ "${CUBE_SANDBOX_CUBE_ROUTER_ENABLE}" == "1" && -n "${CUBE_SANDBOX_CUBE_ROUTER_CIDR}" ]]; then
  check_cidr_preflight "${CUBE_SANDBOX_CUBE_ROUTER_CIDR}" "${cidr_skip_conflict}" "CUBE_SANDBOX_CUBE_ROUTER_CIDR"
