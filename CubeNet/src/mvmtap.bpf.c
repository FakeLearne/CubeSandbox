...
int from_cube(struct __sk_buff *skb)
{
	...
	/* Deliver traffic for the node's primary IPv4 address through cube-dev so
	 * Linux can take the local-input path instead of sending it to an egress
	 * device. The return path is handled by from_envoy on cube-dev egress.
	 */
	if (daddr == nodenic_ip)
		return bpf_redirect(cubegw0_ifindex, BPF_F_INGRESS);
